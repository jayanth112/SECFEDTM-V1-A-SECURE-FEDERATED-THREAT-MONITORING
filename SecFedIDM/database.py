...
def bchain_logact(username,password):
      _conn = sqlite3.connect(user_db_file_location)
      _c = _conn.cursor()
      _c.execute("select * from bc where username='"+username+"' and password='"+password+"'")
      result = _c.fetchall()
      _conn.close()  
      return result
...
def onwer_viewdata(username):
     _conn = sqlite3.connect(user_db_file_location)
     _c = _conn.cursor()
     _c.execute("select filename,owner,hsmsg,pvk,blockkey from cloudadata where owner='"+username+"'")
     result = _c.fetchall()
     _conn.commit()
     _conn.close()
     return result
...
def owner_request(fname,owner,email):
      _conn = sqlite3.connect(user_db_file_location)
      _c = _conn.cursor()
      _c.execute("select hsmsg,pvk,blockkey from cloudadata where owner='"+owner+"' and filename='"+fname+"'")
      result = _c.fetchall()
...
def dele_act(filename):
    _conn = sqlite3.connect(user_db_file_location)
    _c = _conn.cursor()
    i =_c.execute("delete from file where filename='"+filename+"'")
    _conn.commit()
    _conn.close()
    return i

def dele_act1(filename):
    _conn = sqlite3.connect(user_db_file_location)
    _c = _conn.cursor()
    i =_c.execute("delete from cloudadata where filename='"+filename+"'")
    _conn.commit()
    _conn.close()
    return i
...
def check(hsmsg,publickey,bkey):
     _conn = sqlite3.connect(user_db_file_location)
     _c = _conn.cursor()
     ...
     _conn.commit()
     _conn.close()
     if (k==True and i ==True):
        return i
     else:
        return x

def algo(filename,hsmsg,publickey,bkey):
     _conn = sqlite3.connect(user_db_file_location)
     _c = _conn.cursor()
     i =check(hsmsg,publickey,bkey)
     data= ''
